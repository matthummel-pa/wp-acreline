...
Identity, colors, header, typography, and logo: **Appearance → Customize**. Compliance tools (brokerage legal name, Fair Housing, IDX slots, form consent) are under Customize → Compliance or **Appearance → Acreline Settings → Compliance** — not legal advice; see `docs/marketplace/compliance.html`. Homepage Ken Burns photo animation and optional mobile listing-search tilt are under Customize → Header and **Appearance → Acreline Settings → General** (`ks_hero_ken_burns` on, `ks_hero_search_tilt` off) — both screens share the saved values. Marketing pages and posts use the **Gutenberg block editor** (32 custom blocks). Listing, agent, and booking CPTs use classic metaboxes. Editing theme files does not change live posts.
