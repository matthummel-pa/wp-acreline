...
function ks_render_newsletter(array $attrs): string
{
    ...
    $emailLabel = esc_html((string) ($a['emailLabel'] ?? __('Email', 'acreline')));
    ...
    $layout = sanitize_key((string) ($a['layout'] ?? 'split'));
    if (! in_array($layout, ['split', 'stacked', 'inline'], true)) {
        $layout = 'split';
    }
    ...
    $showMarkets = ($a['showMarkets'] ?? true) !== false;
    $showIcon = ($a['showIcon'] ?? true) !== false;
    $markets = array_values(array_filter(array_map('trim', explode(',', (string) ($a['markets'] ?? '')))));
    ...
    $statusId = function_exists('wp_unique_id') ? wp_unique_id('ks-news-status-') : 'ksNewsletterConfirm';

    ob_start();
    ?>
    <section class="<?php echo esc_attr($sectionClass); ?>"<?php echo ks_section_id_attr($a); ?> aria-labelledby="<?php echo esc_attr($hid); ?>">
      <div class="wrap">
        <div class="ks-newsletter ks-band--<?php echo esc_attr($band); ?> ks-news--<?php echo esc_attr($layout); ?> ks-news--form-<?php echo esc_attr($formStyle); ?>">
          <div class="ks-newsletter__glow" aria-hidden="true"></div>
          <div class="ks-newsletter__inner">
            <div class="ks-newsletter__copy">
              <div class="ks-newsletter__kicker">
                <?php if ($showIcon) { ?>
                  <span class="ks-newsletter__icon" aria-hidden="true">
                    <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M4 7l8 6 8-6"/></svg>
                  </span>
                <?php } ?>
                <?php if ($eyebrow !== '') { ?><p class="eyebrow"><?php echo $eyebrow; ?></p><?php } ?>
              </div>
              <<?php echo $hTag; ?> id="<?php echo esc_attr($hid); ?>"><?php echo $title; ?></<?php echo $hTag; ?>>
              <?php if ($text !== '') { ?><p><?php echo $text; ?></p><?php } ?>
              <?php if ($showMarkets && $markets !== []) { ?>
                <ul class="ks-newsletter__markets">
                  <?php foreach ($markets as $market) { ?>
                    <li><?php echo esc_html($market); ?></li>
                  <?php } ?>
                </ul>
              <?php } ?>
            </div>
            <form class="ks-newsletter__form" aria-labelledby="<?php echo esc_attr($hid); ?>">
              <div class="ks-newsletter__fields">
                <div class="field">
              ...
              <p class="ks-newsletter__note" id="<?php echo esc_attr($statusId); ?>" role="status" aria-live="polite"><?php echo $note; ?></p>
            </form>
          </div>
        </div>
      </div>
