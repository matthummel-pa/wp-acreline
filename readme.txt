...
* public/images/partners/*.svg (fictional partner marks for the logo strip)
...
* This is a contrast fix, not a claim that the theme is fully WCAG certified.

= 1.5.3 =
* The top bar defaults to Accent so it tracks the active color style (Forest, Harvest, Orchard, Clay). Sites that already saved Dark, Light, or Custom keep that choice.
* Identity color tokens load after the compiled stylesheet so the bar follows the scheme instead of staying ink.
...
* Home newsletter band redesigned (split / stacked / inline) and follows the color scheme.
* Agents page: new Team Intro block under the hero — desks, snapshot stats, and richer office copy.
* Custom Gutenberg blocks gain consistent band, heading, and spacing controls plus HTML anchors — settings now apply on the front.
* After you install the zip, run Tools → Migrate to Blocks → Force rebuild so Home and Agents pick up the new stacks.
